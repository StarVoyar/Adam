...
class TrainingSchedule:
    def __init__(self, total_steps: int, warmup: int, min_lr: float, base_lr: float, eval_every: int, ckpt_every: int):
    ...
    def lr(self, step: int) -> float:
        ...
        p = (step - self.warmup) / max(1, self.total_steps - self.warmup)
    ...
    def should_ckpt(self, step: int) -> bool:
...
prefetch = 2 if use_cuda else None
...
if use_cuda:
    model = cast(Module, torch.compile(model, mode="max-autotune", fullgraph=True))
...
while True:
    ...
    for x, y in tqdm(train_loader, desc=f"epoch {epoch}", ncols=100):
        if schedule.finished(step):
            break
        x = x.to(device, non_blocking=True)
        y = y.to(device, non_blocking=True)
        for g in optimizer.param_groups:
            g["lr"] = schedule.lr(step)
        ...
        if schedule.should_ckpt(step):
            ...
            save_checkpoint(
                ...
                val_loss=val_loss,
                best_val_loss=best,
                configs={"model": MODEL_CONFIG.__dict__, "tokenizer": TOKENIZER_CONFIG.__dict__},
            )
    if schedule.finished(step):
        break
    epoch += 1
